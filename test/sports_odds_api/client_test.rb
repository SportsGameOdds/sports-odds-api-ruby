...
class SportsOddsAPITest < Minitest::Test
  ...
  def test_client_default_request_default_retry_attempts
    stub_request(:get, "http://localhost/events/?apiKey").to_return_json(status: 500, body: {})

    sports_game_odds =
      SportsOddsAPI::Client.new(base_url: "http://localhost", api_key_header: "My API Key Header")

    assert_raises(SportsOddsAPI::Errors::InternalServerError) do
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(SportsOddsAPI::Errors::InternalServerError) do
      sports_game_odds.events.get(request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(SportsOddsAPI::Errors::InternalServerError) do
      sports_game_odds.events.get(request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:get, "http://localhost/events/?apiKey").to_return_json(
      status: 500,
  ...
  def test_omit_retry_count_header
    ...
    assert_raises(SportsOddsAPI::Errors::InternalServerError) do
      sports_game_odds.events.get(request_options: {extra_headers: {"x-stainless-retry-count" => nil}})
    end
  ...
  def test_overwrite_retry_count_header
    ...
    assert_raises(SportsOddsAPI::Errors::InternalServerError) do
      sports_game_odds.events.get(request_options: {extra_headers: {"x-stainless-retry-count" => "42"}})
    end
  ...
  def test_client_redirect_307
    ...
    assert_raises(SportsOddsAPI::Errors::APIConnectionError) do
      sports_game_odds.events.get(request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: SportsOddsAPI::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: SportsOddsAPI::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_keep_same_origin
    ...
    assert_raises(SportsOddsAPI::Errors::APIConnectionError) do
      sports_game_odds.events.get(request_options: {extra_headers: {"authorization" => "Bearer xyz"}})
    end
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: SportsOddsAPI::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:get, "http://localhost/events/?apiKey").to_return_json(status: 200, body: {data: {}})
